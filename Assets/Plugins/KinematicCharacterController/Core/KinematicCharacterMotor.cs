...
#pragma warning restore 0414 
        ...
        private bool InternalCharacterMove(ref Vector3 transientVelocity, float deltaTime)
        {
            ...
            for (int i = 0; i < _overlapsCount; i++)
            {
                ...
                {
                    bool stableOnHit = IsStableOnNormal(overlapNormal) && !MustUnground() && StableGroundLayers == (StableGroundLayers | (1 << _overlaps[i].Collider.gameObject.layer));
        ...
        public virtual void HandleVelocityProjection(ref Vector3 velocity, Vector3 obstructionNormal, bool stableOnHit)
        {
            ...
            else
            {
                ...
                {
                    // BRYNS EDITS
                    // don't project upwards here
                    Vector3 oldVelocity = velocity;
                    velocity = Vector3.ProjectOnPlane(velocity, obstructionNormal);
                    float upwardsAngle = Vector3.Angle(Vector3.up, obstructionNormal);
                    if (upwardsAngle > MaxStableSlopeAngle)
                    {
                        if (oldVelocity.y > 0)
                        {
                            velocity.y = Mathf.Min(velocity.y, oldVelocity.y);
                        }
                        else
                        {
        ...
        public void EvaluateHitStability(Collider hitCollider, Vector3 hitNormal, Vector3 hitPoint, Vector3 atCharacterPosition, Quaternion atCharacterRotation, Vector3 withCharacterVelocity, ref HitStabilityReport stabilityReport)
        {
            ...
            stabilityReport.IsStable = this.IsStableOnNormal(hitNormal) && StableGroundLayers == (StableGroundLayers | (1 << hitCollider.gameObject.layer));
                            ...
                            true) > 0)
                    {
                        if (IsStableOnNormal(outerSlopeHit.normal))
                        {
                                    ...
                                    <= 0)
                            {
                                ...
                                {
                                    // BRYNS EDITS
                                    // Added minor step skin to fix the stepping up issues
                                    Vector3 stepSkin = new Vector3(0.0f, 0.01f, 0.0f);
                                    // At inner step of the step point
                                    if (CharacterCollisionsRaycast(
                                            farthestHit.point + (innerHitDirection * SecondaryProbesHorizontal) + stepSkin,
                                            -characterUp - stepSkin,
                                            MaxStepHeight,
                                            out innerStepHit,
                                            _internalCharacterHits,
                                            true) > 0)
